...
class EditGamePage extends Component {
      ...
      axios
        ...
        .then(gameResponse => {
          ...
          this.setState({
            ...
            workingDays: game.workingDays,
            visitingHoursFrom: game.visitingHoursFrom,
            visitingHoursTo: game.visitingHoursTo
          });
          document.getElementById('gameName').value = this.state.name;
          document.getElementById('gamePhone').value = this.state.phoneNumber;
  ...
  editGameHandler = event => {
    ...
    if (this.props.match.params.mode === 'edit') {
      request = axios.patch(
        'http://localhost:3100/games/' + this.props.match.params.id,
        gameData
      );
    } else {
      request = axios.post('http://localhost:3100/games', gameData);
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Editing the game details failed. Please try again later'
        );
      });
    specializationArr = [];
    workingDaysArr = [];
  ...
  resetValues = event => {
    ...
    request
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Resetting the game details failed. Please try again later'
        );
      });
  };
  ...
  render() {
    let content = (
      <div>
        <form className="add-form" onSubmit={this.editGameHandler}>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="gameName"
                            placeholder="Enter name of the Game" onChange={event => this.inputChangeHandler(event, 'name')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="gamePhone"
            ...
            <tbody>
              <tr>
                <td style={{ textAlign: "right" }}>
                  <input type="submit" id="btnRegister" className="blueBtn" value="Update Details" onClick={this.editGameHandler} />
                </td>
                <td></td>
                <td style={{ textAlign: "left" }}>
