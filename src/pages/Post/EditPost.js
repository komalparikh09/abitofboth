...
class EditPostPage extends Component {
    ...
    if (this.props.match.params.mode === 'edit') {
      axios
        ...
        .then(postResponse => {
          ...
          document.getElementById('postTitle').value = this.state.title;
          document.getElementById('postCaption').value = this.state.caption;
          //this.editLoadSpecialization(this.state.specialization);
          document.getElementById('postTags').value = this.state.tags;
          //this.editLoadWorkingDays(this.state.workingDays);
          document.getElementById('postPostedBy').value = this.state.postedBy;
          document.getElementById('postPostedDate').value = this.state.postedDate;
          document.getElementById('postPostedTime').value = this.state.postedTime;
          document.getElementById('postImage').value = this.state.image;
          document.getElementById('postVideo').value = this.state.video;
        })
        .catch(err => {
  ...
  editPostHandler = event => {
    ...
    //   this.state.tags.trim() === '' ||
    //   this.state.postedBy.trim() === '' ||
    ...
    if (this.props.match.params.mode === 'edit') {
      request = axios.patch(
        'http://localhost:3100/posts/' + this.props.match.params.id,
        postData
      );
    } else {
      request = axios.post('http://localhost:3100/posts', postData);
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Editing the post details failed. Please try again later'
        );
      });
    specializationArr = [];
    workingDaysArr = [];
  ...
  resetValues = event => {
    ...
    request
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Resetting the post details failed. Please try again later'
        );
      });
  };
  ...
  render() {
    let content = (
      <div>
        <form className="add-form" onSubmit={this.editPostHandler}>
                  ...
                  <table border="0" cellSpacing="10" cellPadding="10" width="100%">
                    ...
                    <tr>
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="postPostedBy"
                            placeholder="Enter your name" onChange={event => this.inputChangeHandler(event, 'postedBy')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="postTitle"
                            placeholder="Enter a title for your post" onChange={event => this.inputChangeHandler(event, 'title')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="postCaption"
                            placeholder="Caption it" onChange={event => this.inputChangeHandler(event, 'caption')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="postTags"
                            placeholder="Mention some tags" onChange={event => this.inputChangeHandler(event, 'tags')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
            ...
            <tbody>
              <tr>
                <td style={{ textAlign: "right" }}>
                  <input type="submit" id="btnRegister" className="blueBtn" value="Update Details" onClick={this.editPostHandler} />
                </td>
                <td></td>
                <td style={{ textAlign: "left" }}>
