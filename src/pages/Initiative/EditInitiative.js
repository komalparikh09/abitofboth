...
class EditInitiativePage extends Component {
  state = {
    isLoading: true,
    name: '',
    phoneNumber: '',
      ...
      axios
        ...
        .then(initiativeResponse => {
          ...
          this.setState({
            ...
            phoneNumber: initiative.phoneNumber,
            specialization: initiative.specialization,
            totalExperience: initiative.totalExperience,
            workingDays: initiative.workingDays,
            visitingHoursFrom: initiative.visitingHoursFrom,
            visitingHoursTo: initiative.visitingHoursTo
          });
          document.getElementById('initiativeName').value = this.state.name;
          document.getElementById('initiativePhone').value = this.state.phoneNumber;
  ...
  editInitiativeHandler = event => {
    ...
    if (this.props.match.params.mode === 'edit') {
      request = axios.patch(
        'http://localhost:3100/initiatives/' + this.props.match.params.id,
        initiativeData
      );
    } else {
      request = axios.post('http://localhost:3100/initiatives', initiativeData);
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
        this.props.history.replace('/initiatives');
      })
      .catch(err => {
        this.setState({ isLoading: false });
        console.log(err);
        this.props.onError(
          'Editing the initiative details failed. Please try again later'
        );
      });
    specializationArr = [];
    workingDaysArr = [];
  ...
  resetValues = event => {
    ...
    request
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Resetting the initiative details failed. Please try again later'
        );
      });
  };
  ...
  render() {
    let content = (
      <div>
        <form className="add-form" onSubmit={this.editInitiativeHandler}>
          <h2>Initiative Registration</h2>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="initiativeName"
                            placeholder="Enter name of the Initiative" onChange={event => this.inputChangeHandler(event, 'name')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="initiativePhone"
            ...
            <tbody>
              <tr>
                <td style={{ textAlign: "right" }}>
                  <input type="submit" id="btnRegister" className="blueBtn" value="Update Details" onClick={this.editInitiativeHandler} />
                </td>
                <td></td>
                <td style={{ textAlign: "left" }}>
