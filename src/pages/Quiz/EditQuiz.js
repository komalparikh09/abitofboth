...
class EditQuizPage extends Component {
      ...
      axios
        ...
        .then(quizResponse => {
          ...
          this.setState({
            ...
            totalExperience: quiz.totalExperience,
            workingDays: quiz.workingDays,
            visitingHoursFrom: quiz.visitingHoursFrom,
            visitingHoursTo: quiz.visitingHoursTo
          });
          document.getElementById('quizName').value = this.state.name;
          document.getElementById('quizPhone').value = this.state.phoneNumber;
  ...
  editQuizHandler = event => {
    ...
    if (this.props.match.params.mode === 'edit') {
      request = axios.patch(
        'http://localhost:3100/quizzes/' + this.props.match.params.id,
        quizData
      );
    } else {
      request = axios.post('http://localhost:3100/quizzes', quizData);
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
        this.props.history.replace('/quizzes');
      })
      .catch(err => {
        this.setState({ isLoading: false });
        console.log(err);
        this.props.onError(
          'Editing the quiz details failed. Please try again later'
        );
      });
    specializationArr = [];
    workingDaysArr = [];
  ...
  resetValues = event => {
    ...
    request
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Resetting the quiz details failed. Please try again later'
        );
      });
  };
  ...
  render() {
    let content = (
      <div>
        <form className="add-form" onSubmit={this.editQuizHandler}>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="quizName"
                            placeholder="Enter name of the Quiz" onChange={event => this.inputChangeHandler(event, 'name')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="quizPhone"
            ...
            <tbody>
              <tr>
                <td style={{ textAlign: "right" }}>
                  <input type="submit" id="btnRegister" className="blueBtn" value="Update Details" onClick={this.editQuizHandler} />
                </td>
                <td></td>
                <td style={{ textAlign: "left" }}>
