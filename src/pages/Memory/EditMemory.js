...
class EditMemoryPage extends Component {
      ...
      axios
        ...
        .then(memoryResponse => {
          ...
          this.setState({
            ...
            totalExperience: memory.totalExperience,
            workingDays: memory.workingDays,
            visitingHoursFrom: memory.visitingHoursFrom,
            visitingHoursTo: memory.visitingHoursTo
          });
          document.getElementById('memoryName').value = this.state.name;
          document.getElementById('memoryPhone').value = this.state.phoneNumber;
  ...
  editMemoryHandler = event => {
    ...
    if (this.props.match.params.mode === 'edit') {
      request = axios.patch(
        'http://localhost:3100/memories/' + this.props.match.params.id,
        memoryData
      );
    } else {
      request = axios.post('http://localhost:3100/memories', memoryData);
    }
    request
      .then(result => {
        this.setState({ isLoading: false });
        this.props.history.replace('/memories');
      })
      .catch(err => {
        this.setState({ isLoading: false });
        console.log(err);
        this.props.onError(
          'Editing the memory details failed. Please try again later'
        );
      });
    specializationArr = [];
    workingDaysArr = [];
  ...
  resetValues = event => {
    ...
    request
      ...
      .catch(err => {
        ...
        console.log(err);
        this.props.onError(
          'Resetting the memory details failed. Please try again later'
        );
      });
  };
  ...
  render() {
    let content = (
      <div>
        <form className="add-form" onSubmit={this.editMemoryHandler}>
          <h2>Memory Registration</h2>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="memoryName"
                            placeholder="Enter name of the Memory" onChange={event => this.inputChangeHandler(event, 'name')} />
                        </td>
                      </tr>
                      <tr>
                        <td className="formFirstCol">
                        ...
                        <td style={{ textAlign: "left" }}>
                          <input type="text" className="textInput" id="memoryPhone"
            ...
            <tbody>
              <tr>
                <td style={{ textAlign: "right" }}>
                  <input type="submit" id="btnRegister" className="blueBtn" value="Update Details" onClick={this.editMemoryHandler} />
                </td>
                <td></td>
                <td style={{ textAlign: "left" }}>
